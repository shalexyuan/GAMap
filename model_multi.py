...
import envs.utils.depth_utils_multi as du
...
class Semantic_Mapping(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.init_grid = torch.zeros(
            args.num_processes, 1 + self.num_sem_categories+self.num_atts, vr, vr,
            self.max_height - self.min_height
        ).float().to(self.device)
        self.feat = torch.ones(
            args.num_processes, 1 + self.num_sem_categories+self.num_atts,
    ...
    def forward(self, obs, pose_obs, maps_last, poses_last, eve_angle,goal_found):
        bs, c, h, w = obs.size()
        depth = obs[:, 3, :, :]
        ...
        voxels = du.splat_att_feat_nd(
            self.init_grid * 0., self.feat, XYZ_cm_std,self.num_atts).transpose(2, 3)
        ...
        # print("========== IN SEM MAP ==========")
        # print(np.unique(voxels[:,[-1],...].max(4)[0].detach().cpu().numpy()))

        agent_height_proj = voxels[:,:-self.num_atts,..., min_z:max_z].sum(4)
        agent_height_proj_att = voxels[:,-self.num_atts:,..., min_z:max_z].max(4)[0]
        agent_height_proj=torch.cat([agent_height_proj,agent_height_proj_att],dim=1)
        agent_height_stair_proj = voxels[:,:-self.num_atts,..., mid_z-5:mid_z].sum(4)
        agent_height_stair_proj_att = voxels[:,-self.num_atts:,..., mid_z-5:mid_z].max(4)[0]
        agent_height_stair_proj=torch.cat([agent_height_stair_proj,agent_height_stair_proj_att],dim=1)
        all_height_proj = voxels[:,:-self.num_atts,...].sum(4)
        all_height_proj_att = voxels[:,-self.num_atts:,...].max(4)[0]
        all_height_proj=torch.cat([all_height_proj,all_height_proj_att],dim=1)
        ...
        agent_view[:, 4:-self.num_atts, y1:y2, x1:x2] = torch.clamp(
            agent_height_proj[:, 1:-self.num_atts, :, :] / self.cat_pred_threshold,
            min=0.0, max=1.0)
        agent_view[:, -self.num_atts:, y1:y2, x1:x2] =agent_height_proj[:, -self.num_atts:, :, :]

        # print("SEM MAP: MAX ",agent_height_proj[:, -1, :, :].max(),", MIN: ",agent_height_proj[:, -1, :, :].min())
        # print("SEM MAP/HEIGHT: MAX ",agent_view[:, -1, ...].max(),", MIN: ",agent_view[:, -1, ...].min())
        ...
        # pdb.set_trace()
        if not goal_found:
            # TODO: better way to get the score
            # maps2 = torch.cat((maps_last.unsqueeze(1), translated.unsqueeze(1)), 1)
            _score_maps_last=maps_last[:,-self.num_atts:,...]
            _score_maps_new=translated[:, -self.num_atts:, ...]
            updated_mask=_score_maps_new>0
            _s_map=torch.cat((_score_maps_last.unsqueeze(1), _score_maps_new.unsqueeze(1)), 1)
            _s_map,_=torch.max(_s_map, 1)
            map_score_new = torch.where(updated_mask, _s_map / 2, _score_maps_last)
            maps2 = torch.cat((maps_last[:, :-self.num_atts, ...].unsqueeze(1), translated[:, :-self.num_atts, ...].unsqueeze(1)), 1)
            map_pred, _ = torch.max(maps2, 1)
            map_pred=torch.cat([map_pred,map_score_new],1)
            # print("map_score_new: MAX ",map_score_new.max(),", MIN: ",map_score_new.min())
        else:
            maps2 = torch.cat((maps_last.unsqueeze(1), translated.unsqueeze(1)), 1)
            map_pred, _ = torch.max(maps2, 1)


        # map_pred=torch.cat((_map_pred, map_score_new), dim=1)
        ...
        # print("map_pred_-1: MAX ",map_pred[:,-1,...].max(),", MIN: ",map_pred[:,-1,...].min())
