...
from constants import category_to_id, hm3d_category, category_to_id_gibson,color_palette,cat_att,cat_att_prompt
...
def find_centroid_of_largest_connected(image):
    ...
    maxLabel = np.argmax([props[i].area for i in range(len(props))]) + 1
    resMatrix = (img_label == maxLabel).astype(np.uint8)
    centroid = props[maxLabel - 1].centroid

    return centroid

def find_largest_connected_area(free_map, contours):

    # Create a mask from the contour
    mask = np.zeros_like(free_map)
    cv2.drawContours(mask, contours, -1, color=255, thickness=cv2.FILLED)

    # Apply the mask to the free map
    navigation_space = cv2.bitwise_and(free_map_binary, mask)

    # Find all connected components
    num_labels, labels, stats, centroids = cv2.connectedComponentsWithStats(navigation_space, connectivity=8)
...
def main():
    ...
    nc = args.num_sem_categories + 4+1  # num channels
    ...
    all_target_edge_map = np.zeros((num_scenes, local_w,
                                local_h))
    all_target_point_map = np.zeros((num_scenes, local_w,
                                 local_h))
    ...
    frontier_area_list=[]
    for _ in range(args.num_processes):
        frontier_area_list.append(deque(maxlen=10))

    all_frontier_area_list=[]
    for _ in range(args.num_processes):
        all_frontier_area_list.append(deque(maxlen=10))

    collision_queue=Pos_Queue(args.collision_queue_size)
    ...
    def init_map_and_pose_for_env(e):
        ...
        target_point_map[e] = np.zeros((local_w,
                                        local_h))
        all_target_edge_map[e] = np.zeros((local_w,
                                       local_h))
        all_target_point_map[e] = np.zeros((local_w,
                                        local_h))
        step_masks[e] = 0
        stair_flag[e] = 0
        clear_flag[e] = 0
    ...
    def remove_small_points(local_ob_map, image, threshold_point, pose,step):
        ...
        tmp_ob_img=local_ob_map*255
        tmp_ob_img=Image.fromarray(tmp_ob_img.astype(np.uint8))
        tmp_ob_img.save(os.path.join('/home/mmvc/Desktop/L3MVN_Reproduce/L3MVN/vis/tmp/ob',f"local_ob_{step}.png"))
        ...
        tmp_img.save(os.path.join('/home/mmvc/Desktop/L3MVN_Reproduce/L3MVN/vis/tmp/img',f"image_{step}.png"))
        ...
        Goal_score = []
        dict_cost = {}
        ALL_Goal_edge = np.zeros((img_label.shape[0], img_label.shape[1]))
        ALL_Goal_point = np.zeros(img_label.shape)
        ALL_Goal_score = []
        all_dict_cost = {}
        # print("PROPS LEN: ",len(props))
        # print(props)
        ...
        plt.savefig(os.path.join('/home/mmvc/Desktop/L3MVN_Reproduce/L3MVN/vis/tmp/img_label',f"props_{step}.png"), bbox_inches='tight', pad_inches=0)
        plt.close(fig)
        #=============================
        ...
        for i in range(1, len(props)):
            ...
            #     dict_cost[i] = cost
            if props[i].area > threshold_point and dist > 5 and dist < 500:
                dict_cost[i] = cost
            all_dict_cost[i]=cost
        ...
        if all_dict_cost:
            all_dict_cost = sorted(all_dict_cost.items(), key=lambda x: x[1], reverse=True)

            # print(dict_cost)
            for i, (key, value) in enumerate(all_dict_cost):
                # print(i, key)
                ALL_Goal_edge[img_label == key + 1] = 1
                ALL_Goal_point[int(props[key].centroid[0]), int(props[key].centroid[1])] = i + 1  #
                ALL_Goal_score.append(value)
                if i == 3:
                    break

        return Goal_edge, Goal_point, Goal_score,ALL_Goal_edge,ALL_Goal_point,ALL_Goal_score
    ...
    increase_local_map, local_map, local_map_stair, local_pose = \
        sem_map_module(obs, poses, local_map, local_pose, eve_angle,goal_found)
    ...
    for e, p_input in enumerate(planner_inputs):
        ...
        if args.visualize or args.print_images:
            ...
            p_input['attr_map']=local_map[e, -1, :,:].cpu().numpy()

    # pdb.set_trace()
    # print("planner_inputs: MAX ",planner_inputs[0]['attr_map'].max(),", MIN: ",planner_inputs[0]['attr_map'].min())
    ...
    NEW_EPISODE_C=0
    set_collision_new_goal=0
    go_to_large_cc=0
    go_to_large_cc_n=0
    stuck_n=0
    for step in range((args.num_training_frames+11) // args.num_processes + 1):
        ...
        if l_step == args.num_local_steps - 1:
            ...
            for e in range(num_scenes):
                ...
                # pdb.set_trace()
                #============== VISUALZIE FREE MAP====================
                tmp_free_map = local_map[e][1].cpu().numpy()*255.
                tmp_free_map = Image.fromarray(tmp_free_map.astype(np.uint8))
                tmp_free_map.save(
                    os.path.join('/home/mmvc/Desktop/L3MVN_Reproduce/L3MVN/vis/tmp/fm_update', f"fm_{step}.png"))
                #============================================================

                # pdb.set_trace()

                #==================== VISUALZIE NAV MAP====================
                _, explored_binary = cv2.threshold(local_map[e][1].cpu().numpy()*255, 100, 255, cv2.THRESH_BINARY_INV)
                _, obstacle_binary = cv2.threshold(local_ob_map[e]*255, 100, 255, cv2.THRESH_BINARY)
                # Combine explored map and obstacle map to create the navigation map
                # navigation_map = cv2.bitwise_and(explored_binary, cv2.bitwise_not(obstacle_binary).astype(np.float32))
                navigation_map = 255.-obstacle_binary
                tmp_navigation_space = navigation_map
                tmp_navigation_space = Image.fromarray(tmp_navigation_space.astype(np.uint8))
                tmp_navigation_space.save(
                    os.path.join('/home/mmvc/Desktop/L3MVN_Reproduce/L3MVN/vis/tmp/inv_ob', f"ob_{step}.png"))

                # Apply the contour to the navigation map
                mask = np.zeros_like(navigation_map)
                cv2.drawContours(mask, contours, -1, (255), thickness=cv2.FILLED)
                region_of_interest = cv2.bitwise_and(navigation_map, mask)
                tmp_navigation_space = region_of_interest
                tmp_navigation_space = Image.fromarray(tmp_navigation_space.astype(np.uint8))
                tmp_navigation_space.save(
                    os.path.join('/home/mmvc/Desktop/L3MVN_Reproduce/L3MVN/vis/tmp/nav', f"nav_{step}.png"))
                # ============================================================
                # if step>20:
                #     pdb.set_trace()
                if len(contours) > 0:
                ...
                # ==================== VISUALZIE EX MAP====================
                tmp_local_ex_map = local_ex_map[e] * 255
                tmp_local_ex_map = Image.fromarray(tmp_local_ex_map.astype(np.uint8))
                tmp_local_ex_map.save(
                    os.path.join('/home/mmvc/Desktop/L3MVN_Reproduce/L3MVN/vis/tmp/ex', f"local_ex_{step}.png"))
                # ============================================================

                # ==================== VISUALZIE OB MAP====================
                tmp_local_ob_map = local_ob_map[e] * 255
                tmp_local_ob_map = Image.fromarray(tmp_local_ob_map.astype(np.uint8))
                tmp_local_ob_map.save(
                    os.path.join('/home/mmvc/Desktop/L3MVN_Reproduce/L3MVN/vis/tmp/ad_ob', f"ad_ob_{step}.png"))
                # ============================================================

                target_edge[target_edge > 0.8] = 1.0
                target_edge[target_edge != 1.0] = 0.0

                local_pose_map = [local_pose[e][1] * 100 / args.map_resolution,
                                  local_pose[e][0] * 100 / args.map_resolution]
                # print("REMOVING SMALL CC. STEP: ", step)
                target_edge_map[e], target_point_map[e], Goal_score,all_target_edge_map[e],all_target_point_map[e],\
                    all_scores = remove_small_points(_local_ob_map, target_edge, 4,local_pose_map,step)
                ...
                frontier_area_list[e]=Goal_score
                all_frontier_area_list[e]=all_scores
                ...
                for lay in range(tpm):
                    ...
                    if len(objs_list) > 0 and found_goal[e] == 0:
                        ...
                        frontier_scores = local_map[e][-1, fmb[0]:fmb[1], fmb[2]:fmb[3]].mean()
        ...
        for e in range(num_scenes):
            ...
            # target_score_map=np.zeros((local_w, local_h))
            # score_map = local_map[e, -1, :, :].cpu().numpy()
            # attr_scores = score_map
            # max_attr_score=np.max(score_map)
            # target_score_map[attr_scores ==max_attr_score] = 1.
            # local_goal_maps[e] = find_big_connect(target_score_map)
            # ------------------------------------------------------------------
            # if step>217:
            #     pdb.set_trace()
            cn = infos[e]['goal_cat_id'] + 4
            if local_map[e, cn, :, :].sum() != 0.:
                ...
                found_goal[e] = 1
                # if doubly_right_detection(Image.fromarray(obs[e,:,:,0:3].cpu().numpy().astype(np.uint8)),doubly_model,
                #                           doubly_vis_processors["eval"],cat_att_prompt[infos[e]['goal_name']],b_device):
                #     print("Find the target")
                #     cat_semantic_map = local_map[e, cn, :, :].cpu().numpy()
                #     cat_semantic_scores = cat_semantic_map
                #     cat_semantic_scores[cat_semantic_scores > 0] = 1.
            ...
            # pdb.set_trace()
            if not found_goal[e]:
                if not NEW_EPISODE_C<13:
                    # FROM TRIHELPER: COLLISION HELPER
                    # x1, y1, _, _, _, _, _ = planner_pose_inputs[e]
                    # curr_loc = [x1, y1]
                    # pop_loc=collision_queue.add_element(curr_loc)
                    # if pop_loc is not None:
                    #     x2,y2=pop_loc[0],pop_loc[1]
                    #     dist = pu.get_l2_distance(x1, x2, y1, y2)
                    #     # print(dist)
                    #     if infos[e]['already_coll']==1 and \
                    #             ((dist < args.collision_threshold and infos[e]['last_action'] == 1) or
                    #             infos[e]['fmm_fail'] or
                    #             (dist < args.collision_threshold and go_to_large_cc==1)):
                    #         if (dist < args.collision_threshold and infos[e]['last_action'] == 1) or \
                    #                 (dist < args.collision_threshold and go_to_large_cc == 1) :
                    #             print("STUCK")
                    #             # stuck_n+=1
                    #         if infos[e]['fmm_fail']:
                    #             print("FMM_ERROR")
                    #             # stuck_n =0
                    #         set_collision_new_goal=1
                    #         # if stuck_n<5:
                    #         try:
                    #             print("select max frontier: ",stuck_n)
                    #             local_goal_maps[e] = np.zeros((local_w, local_h))
                    #             fronter_i = np.argmax(np.array(frontier_area_list[e]))
                    #             # print("+++++++++ COLL ++++++++")
                    #             # print(fronter_i)
                    #             # print(np.unique(target_point_map[e]))
                    #             local_goal_maps[e][target_point_map[e] == fronter_i + 1] = 1
                    #         # else:
                    #         except:
                    #             print("select max frontier: ", stuck_n)
                    #             local_goal_maps[e] = np.zeros((local_w, local_h))
                    #             fronter_i = np.argmax(np.array(all_frontier_area_list[e]))
                    #             # print("+++++++++ COLL ++++++++")
                    #             # print(fronter_i)
                    #             # print(np.unique(target_point_map[e]))
                    #             local_goal_maps[e][all_target_point_map[e] == fronter_i + 1] = 1
                    #             # local_map[e].fill_(0.)
                    #             # clear_flag[e] = 0
                    #
                    #             # print("select big cc: ",stuck_n)
                    #             # go_to_large_cc = 1
                    #             # local_ob_map[e] = np.zeros((local_w,
                    #             #                             local_h))
                    #             # local_ex_map[e] = np.zeros((local_w,
                    ...
                    #             # local_ob_map[e] = cv2.dilate(_local_ob_map, kernel)
                    #             #
                    #             # _, explored_binary = cv2.threshold(local_map[e][1].cpu().numpy() * 255, 100, 255,
                    ...
                    #             # mask = np.zeros_like(navigation_map)
                    #             # cv2.drawContours(mask, contours, -1, (255), thickness=cv2.FILLED)
                    #             # region_of_interest = cv2.bitwise_and(navigation_map, mask).astype(np.uint8)
                    #             #
                    #             # # Find the largest connected component in the ROI
                    #             # num_labels, labels, stats, centroids = cv2.connectedComponentsWithStats(region_of_interest,
                    #             #                                                                         connectivity=8)
                    #             # largest_component_index = np.argmax(
                    #             #     stats[1:, cv2.CC_STAT_AREA]) + 1  # Ignore the background
                    #             # centroid = centroids[largest_component_index]
                    #             #
                    #             # local_goal_maps[e][int(centroid[0]), int(centroid[1])] = 1
                    #             #
                    #             # local_ex_map[e] = np.zeros((local_w,
                    #             #                             local_h))
                    #             #
                    ...
                    # #
                    #     else:
                    #         # print("CLEAN COLL HELPER")
                    #         set_collision_new_goal = 0
                    #         stuck_n =0
                    #         go_to_large_cc=0
            #
                    # pdb.set_trace()
                    loc_to_explore = np.argwhere(local_map[e][-1].cpu().numpy() > args.wtec)
                    # if (len(loc_to_explore) > 0):
        ...
        for e, p_input in enumerate(planner_inputs):
            ...
            p_input['map_target'] = target_point_map[e]  # global_goals[e]
            if set_collision_new_goal:
                print("SET COLISOON NEW GOAL")
                p_input['new_goal'] = 1
            else:
        ...
        if NEW_EPISODE_C<13:
            ...
            obs, fail_case, done, infos = envs.rotate_at_beginning_act(planner_inputs)
            NEW_EPISODE_C+=1
        else:
