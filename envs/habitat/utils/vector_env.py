...
INIT_ROTATION = "rotate_at_beginning_act"
...
class VectorEnv:
    ...
    def _worker_env(
        ...
        try:
            ...
            while command != CLOSE_COMMAND:
                ...
                elif command ==INIT_ROTATION:
                    observations, reward, done, info = \
                            env.rotate_at_beginning_act(data)
                    connection_write_fn((observations, reward, done, info))
    ...
    def rotate_at_beginning_act(self, inputs):
        self._assert_not_closed()
        self._is_waiting = True
        for e, write_fn in enumerate(self._connection_write_fns):
            write_fn((INIT_ROTATION, inputs[e]))
        results = []
        for read_fn in self._connection_read_fns:
            results.append(read_fn())
        obs, rews, dones, infos = zip(*results)
