...
def splat_att_feat_nd(init_grid, feat, coords):
    """
    Args:
        init_grid: B X nF X W X H X D X ..
        feat: B X nF X nPt
    ...
    grid_flat = init_grid[:,:-1,...].view(B, F-1, -1)
    grid_flat_att = init_grid[:,-1:,...].view(B,1, -1)
    ...
    for ix_d in itertools.product(*l_ix):
        ...
        index = index.long()
        grid_flat.scatter_add_(2, index.expand(-1, F-1, -1), feat[:,:-1,:] * wts)
        grid_flat = torch.round(grid_flat)

        current_values = grid_flat_att.gather(2, index)
        new_val = torch.max(current_values,  (feat[:,-1:,:] * wts))
        grid_flat_att.scatter_(2, index, new_val)

    final_grid_flat=torch.cat([grid_flat,grid_flat_att],dim=1)

    assert final_grid_flat.shape==init_grid.view(B, F, -1).shape
    return final_grid_flat.view(init_grid.shape)
