...
class Semantic_Mapping(nn.Module):
    ...
    def forward(self, obs, pose_obs, maps_last, poses_last, eve_angle,goal_found):
        ...
        agent_height_proj = voxels[:,:-1,..., min_z:max_z].sum(4)
        agent_height_proj_att = voxels[:,[-1],..., min_z:max_z].max(4)[0]
        agent_height_proj=torch.cat([agent_height_proj,agent_height_proj_att],dim=1)
        agent_height_stair_proj = voxels[:,:-1,..., mid_z-5:mid_z].sum(4)
        agent_height_stair_proj_att = voxels[:,[-1],..., mid_z-5:mid_z].max(4)[0]
        ...
        all_height_proj_att = voxels[:,[-1],...].max(4)[0]
        ...
        agent_view[:, 4:agent_view.shape[1]-1, y1:y2, x1:x2] = torch.clamp(
            agent_height_proj[:, 1:agent_height_proj.shape[1]-1, :, :] / self.cat_pred_threshold,
            min=0.0, max=1.0)
        agent_view[:, -1:, y1:y2, x1:x2] =agent_height_proj[:, -1:, :, :]
        ...
        if not goal_found:
            ...
            _score_maps_last=maps_last[:,[-1],...]
            _score_maps_new=translated[:, [-1], ...]
            ...
            maps2 = torch.cat((maps_last[:, :-1, ...].unsqueeze(1), translated[:, :-1, ...].unsqueeze(1)), 1)
