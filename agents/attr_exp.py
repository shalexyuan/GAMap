...
from constants import color_palette,cat_att,cat_att_prompt
...
class Attr_Exp_Env_Agent(ObjectGoal_Env21):
    ...
    def __init__(self, args, rank, config_env, dataset):
        ...
        self.blip_device = torch.device(f"cuda:{args.blip_gpu_id}") if torch.cuda.is_available() else "cpu"

        #--------BLIP-------
        # self.blip_model, self.vis_processors, self.text_processors = \
        #     load_model_and_preprocess("blip2_image_text_matching", "pretrain",device=self.blip_device, is_eval=True)

        #--------CLIP-------
        self.blip_model, self.vis_processors, self.text_processors = \
            load_model_and_preprocess("clip_feature_extractor",model_type="ViT-B-32",is_eval=True, device=self.blip_device)

        # --------ATT-------
        # self.blip_model, self.vis_processors, self.text_processors = load_model_and_preprocess("blip_image_text_matching", "large",
        #                                                                    device=self.blip_device, is_eval=True)
    ...
    def reset(self):
        ...
        # sys.exit()
        goal=info['goal_name']
        self.goal_attributes=[cat_att_prompt[goal]]
        # print(self.goal_attributes)
        # sys.exit()
    ...
    def rotate_at_beginning_act(self, planner_inputs):
        ...
        if planner_inputs["new_goal"]:
            ...
            # self.info['already_coll']=0
        if planner_inputs['init_rotate_flag']==2:
            action = 5
        elif planner_inputs['init_rotate_flag']==0:
            action = 4
        else:
            action=3
        # c_time = time.time()
        ...
        if self.collision_n > 20 or self.replan_count > 26:
            ...
            self.collision_n = 0
        # if self.collision_n > 10 or self.replan_count > 26:
        #     self.info['collision_helper'] = 1
        #     self.collision_n = 0
    ...
    def plan_act_and_preprocess(self, planner_inputs):
        ...
        if planner_inputs['NEW_EPISODE_C'] < 16:
            print(f"INIT ROTATE")
            if planner_inputs['init_rotate_flag'] == 2:
                action = 5
            elif planner_inputs['init_rotate_flag'] == 0:
        ...
        if action >= 0:
            ...
            #     if info['time'] >= self.args.max_episode_length - 1:
            #         self.fail_case['exploration'] += 1
            #     elif self.replan_count > 26:
            #         self.fail_case['collision'] += 1
            #     else:
            #         self.fail_case['detection'] += 1
            # if done and info['success'] == 1:
            #     self.fail_case['success'] += 1
            ...
            self.last_action = action['action']
            self.obs = obs
            # if 'last_action' not in info.keys():
                # info['last_action']=-1
            # info['lastoflast_action'] = info['last_action']  # Updated to get action info as well
            info['last_action'] = action['action']  # Updated to get action info as well
            self.info = info
            info['eve_angle'] = self.eve_angle
    ...
    def _plan(self, planner_inputs):
        ...
        if replan:
            ...
            if self.replan_count>18:
                self.info['fmm_fail']=1
        else:
            self.info['fmm_fail'] = 0
            self.replan_count = 0
    ...
    def _preprocess_obs(self, obs, use_seg=True):
        ...
        # ------------------BLIP------------------
        # _,score_map=get_permird_score(Image.fromarray(obs[...,0:3].astype(np.uint8)),
        #                                   self.vis_processors["eval"],self.text_processors,self.blip_model,self.goal_attributes,
        #                                   self.blip_device)

        # ------------------CLIP------------------
        _,score_map=get_pyramid_clip_score(Image.fromarray(obs[...,0:3].astype(np.uint8)),
                                          self.vis_processors["eval"],self.text_processors["eval"],self.blip_model,
                                           self.goal_attributes,self.blip_device)

        # ------------------ATT------------------
        # score_map=get_att_score(Image.fromarray(obs[...,0:3].astype(np.uint8)),
        #                                   self.vis_processors["eval"],self.text_processors["eval"],self.blip_model,self.goal_attributes,
        #                                   self.blip_device)

        score=score_map[self.goal_attributes[0]][...,None]
        # print("========== IN AGENT ==========")
        # print(np.unique(score))
        # t=time.time()
        # fn=f"/home/mmvc/Desktop/L3MVN_Reproduce/L3MVN/vis/3d_score/obs_score{t}.png"
        # overlay_attention_map(Image.fromarray(obs[...,0:3].astype(np.uint8)),score,fn)
        #------------------------------------------------------
        ...
        if ds != 1:
            ...
            score = score[ds // 2::ds, ds // 2::ds]

        depth = np.expand_dims(depth, axis=2)
        state = np.concatenate((rgb, depth, sem_seg_pred,score),
    ...
    def _visualize(self, inputs):
        ...
        score_dir = '{}/episodes/thread_{}/score_{}/'.format(
            dump_dir, self.rank, self.episode_no)
        # ori_sem_dir = '{}/episodes/thread_{}/ori_sem_{}/'.format(
        #     dump_dir, self.rank, self.episode_no)
        ...
        # if not os.path.exists(ori_sem_dir):
        #     os.makedirs(ori_sem_dir)
        ...
        #
        # sem_ori_map_fn = '{}/episodes/thread_{}/ori_sem_{}/{}-{}-Vis-{}.png'.format(
        #     dump_dir, self.rank, self.episode_no,
        #     self.rank, self.episode_no, self.timestep)
        # img_array = np.array(sem_map)
        # img_array = np.flipud(img_array)
        # plt.figure(figsize=(10, 6))
        # plt.imshow(img_array, cmap='gray', interpolation='nearest')  # Display the original image
        # plt.axis('off')  # Hide axes
        # # Save the figure
        # plt.savefig(sem_ori_map_fn, bbox_inches='tight', pad_inches=0)
        ...
        # save attr_map
        scores_map=inputs['attr_map']
        scores_map=np.flipud(scores_map)
